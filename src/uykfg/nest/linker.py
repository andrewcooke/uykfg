...
class Linker:
    ...
    def link(self, session, src, links, max_links=8):
        try:
            id = session.query(NestArtist).join('artists').filter(Artist.id == src.id).one().id
            start, results, found = 0, 6, 0
            while links < max_links and results < 100:
                links = self._link_delta(session, src, start, results, links, max_links, id)
                start = results
                results *= 2
    ...
    def _link_delta(self, session, src, start, results, links, max_links, id):
        commit = False
